...
export function ElasticScript({ initialText, sceneDuration, onTextChange }: ElasticScriptProps) {
    ...
    const status = estimatedTime <= sceneDuration ? 'safe' : estimatedTime <= sceneDuration + 1.5 ? 'tight' : 'overflow';
    ...
    return (
        <div className="bg-[var(--color-bg-secondary)] p-4 rounded-xl border border-[var(--color-border)] text-[var(--color-text-primary)] font-sans shadow-lg">
            <div className="flex justify-between items-center mb-2">
                ...
                <span className={`text-xs font-mono px-2 py-1 rounded-md font-bold transition-colors border ${status === 'safe' ? 'bg-green-500/20 text-green-400 border-green-500/30' :
                    status === 'tight' ? 'bg-yellow-500/20 text-yellow-400 border-yellow-500/30' :
            ...
            <div className="relative h-8 bg-[var(--color-bg-tertiary)] rounded mb-4 overflow-hidden flex flex-col gap-1 p-1 border border-[var(--color-border)]">
                ...
                <div className="h-1/2 w-full bg-[var(--color-accent-primary)]/20 rounded-sm relative">
                    <span className="absolute left-1 top-0 text-[8px] text-[var(--color-accent-primary)] font-bold tracking-wider">VIDEO SCENE ({sceneDuration}s)</span>
                </div>

                {/* Audio Duration Bar */}
                <div className="h-1/2 bg-[var(--color-bg-primary)]/50 rounded-sm relative w-full overflow-hidden">
                    <div
                        className={`h-full transition-all duration-300 ${status === 'safe' ? 'bg-green-500/60' :
                            status === 'tight' ? 'bg-yellow-500/60' :
                                'bg-red-500/60'
                            }`}
                        style={{ width: `${Math.min((estimatedTime / sceneDuration) * 100, 100)}%` }}
                    />
                    <span className="absolute left-1 top-0 text-[8px] text-[var(--color-text-secondary)] font-bold opacity-75">NARRATION</span>
                </div>
            </div>
