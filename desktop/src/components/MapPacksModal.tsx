...
import { Map, X, Check, Download, Trash2, RefreshCw } from 'lucide-react';
        ...
        {loading ? (
          ...
          <div className="region-list">
            ...
            {regions.map((region) => (
              <div
                key={region.id}
                className={`region-item ${region.downloaded ? 'downloaded' : ''}`}
              >
                <div className="region-info">
                  <div className="region-name">
                    {region.downloaded && (
                      <Check className="w-4 h-4 text-[var(--color-success)] inline-block mr-2" />
                    )}
                    {region.name}
                  </div>
