...
export function TruthVerification({
  ...
  events,
  onVerifyEvent,
  onAdjustTimestamp: _onAdjustTimestamp,
}: TruthVerificationProps) {
      ...
      <div className="h-1/2 flex flex-col bg-[var(--color-bg-secondary)] relative">
        ...
        <div className="p-3 border-b border-[var(--color-border)] flex justify-between items-center bg-[var(--color-bg-tertiary)]/50 backdrop-blur-sm z-10">
          ...
          <div className="flex gap-2">
            <button
              className={`text-[10px] font-medium px-3 py-1.5 rounded-md border transition-all ${
                lockToFrame
            ...
            return (
              ...
              >
                ...
                <div className="w-16 flex flex-col items-center justify-center border-r border-[var(--color-border)] pr-4 relative">
                  <span
                    className={`font-mono text-sm tracking-tight ${isActive ? 'text-[var(--color-accent-primary)] font-bold' : 'text-[var(--color-text-muted)]'}`}
                  >
                    {new Date(event.timestamp * 1000).toISOString().substr(14, 5)}
                  </span>
                  <Clock
                    size={12}
                    className={`mt-1.5 ${isActive ? 'text-[var(--color-accent-primary)]' : 'text-[var(--color-text-muted)] opacity-50'}`}
                  />

                  {/* Timeline dot */}
                  <div
                    className={`absolute -right-[5px] top-1/2 -translate-y-1/2 w-2.5 h-2.5 rounded-full border-2 border-[var(--color-bg-secondary)] ${isActive ? 'bg-[var(--color-accent-primary)]' : 'bg-[var(--color-border)]'}`}
                  />
                </div>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1.5">
                    ...
                    {isActive && (
                      <span className="text-[9px] text-[var(--color-accent-primary)] animate-pulse font-bold flex items-center gap-1">
                        ● ON AIR
                      </span>
                    )}
                  </div>
                  ...
                  <p
                    className={`text-sm leading-relaxed ${isActive ? 'text-[var(--color-text-primary)]' : 'text-[var(--color-text-secondary)]'}`}
                  >
                    {event.content}
                  </p>
                </div>
                ...
                <div
                  className={`flex flex-col justify-center gap-2 ${isActive ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'} transition-opacity p-1`}
                >
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      onVerifyEvent?.(event.id, !event.verified);
                    }}
