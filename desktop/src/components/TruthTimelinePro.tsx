...
import {
  Timeline,
  TimelineEffect,
  type TimelineRow as TimelineRowType,
...
export function TruthTimelinePro({
  duration: _duration,
  events,
  narrationTracks,
      ...
      <div className="flex-1 overflow-auto custom-timeline-wrapper">
        <Timeline
          ...
          getActionRender={(action, _row) => {
            ...
            return (
              <div
                className={`h-full w-full rounded flex items-center px-2 text-xs truncate overflow-hidden cursor-move ${isGps ? 'bg-blue-600 border border-blue-400' : 'bg-emerald-600 border border-emerald-400'}`}
              >
