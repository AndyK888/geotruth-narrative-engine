...
export function EditorPage({ videoPath, onBack }: EditorPageProps) {
    ...
    const [narration, _setNarration] = useState([
        { id: 'n1', action: 'narration', startTime: 6, endTime: 14, text: 'Looking out over the coastline...' }
    ]);

    // Derived events for verification
    const verificationEvents = [
        ...events.map(e => ({ id: e.id, timestamp: e.startTime, type: 'gps' as const, content: e.text, verified: true })),
        ...narration.map(e => ({ id: e.id, timestamp: e.startTime, type: 'narration' as const, content: e.text || "", verified: false }))
    ].sort((a, b) => a.timestamp - b.timestamp);

    const handleMomentCaptured = (data: { timestamp: number; image: string; description?: string }) => {
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <div className="text-xs text-[var(--color-text-secondary)] flex items-center gap-2">
                            ...
                            <span className="font-mono text-[var(--color-accent-primary)]">{videoPath.split('/').pop()}</span>
                        </div>
                    </div>
                </div>
                            ...
                            <div className="flex-1 overflow-y-auto pr-2 space-y-3 custom-scrollbar">
                                {capturedMoments.length === 0 ? (
                                    ...
                                    capturedMoments.map((moment) => (
                                        <div key={moment.id} className="bg-[var(--color-bg-secondary)] border border-[var(--color-border)] rounded-lg p-3 hover:border-[var(--color-accent-primary)] transition-all group">
                                            <div className="flex gap-3">
                                                <div className="w-24 h-16 bg-black rounded shrink-0 overflow-hidden relative">
                                                    <img src={moment.image} alt="Capture" className="w-full h-full object-cover opacity-80 group-hover:opacity-100 transition-opacity" />
